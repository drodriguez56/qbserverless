...
var Tokens = __webpack_require__(4);
var csrf = new Tokens();
...
var _csrf = __webpack_require__(4);
  ...
  (0, _request2.default)(requestObj, function (err, response) {
    ...
    _tools2.default.checkForUnauthorized(event.body, requestObj, err, response).then(function (_ref) {
      ...
      db.once("open", function () {
        _User2.default.findOneOrCreate({
          email: data.email,
          ...
          _Company2.default.findById(companyId).populate({
            path: "applications",
            populate: {
              path: "user",
              model: "user"
            ...
            var hasApplied = company.applications.filter(function (appl) {
              return appl.user._id.toString() === user._id.toString();
            });
            console.log(hasApplied.length);
            if (hasApplied.length > 0) {
              return callback(null, createErrorResponse(422, "already applied"));
            } else {
              var application = new _Application2.default({ company: company, user: user });
              user.applications.push(application);
              company.applications.push(application);
              return _promise2.default.all([user.save(), application.save(), company.save()]).then(function () {
...
UserSchema.statics.findOneOrCreate = function (user, cb) {
  var self = this;
  self.findOne({ email: user.email }, function (err, result) {
    return result ? cb(err, result) : self.create(user, function (err, result) {
